/* =============================================
   BUSINESS RECORDS MANAGER — app.js  (v6 Firebase)
   ============================================= */
'use strict';
...
async function initApp() {
  try {
    const { initFirebase, onAuthStateChanged, signOut,
    ...
    onAuthStateChanged(auth, user => {
      ...
      // Real-time listener — replaces localStorage load()
      ...
      // Expose CRUD helpers globally so existing handlers work
      window._fbSave = async (r) => {
        const { id, ...data } = r;
        if (id && id.length > 10) {
          // Firestore doc id (not our old uid format)
    ...
    if (!sessionStorage.getItem('bizAuthed')) { location.replace('./login.html'); }
    load_local();
    renderTable();
    updateStats();
    window._fbSave = async (r) => save_local();
    window._fbDelete = async (id) => {
...
function activateCell(td) {
  ...
  input.focus();
  if (input.type !== 'date' && input.select) input.select();

  // Commit on blur (with small delay so dropdown click can fire first)
  input.addEventListener('blur', () => setTimeout(() => commitCell(td, id, key), 120));

  // Keyboard nav
  input.addEventListener('keydown', e => {
    // If autocomplete is open, handle arrow keys for it
    ...
    if (drop && !drop.classList.contains('hidden')) {
      ...
      if (e.key === 'Enter') {
        const sel = drop.querySelector('.ac-item.ac-active');
        if (sel) { e.preventDefault(); acSelectValue(sel.textContent, key, id, td); return; }
      }
    ...
    if (e.key === 'Enter' || e.key === 'Tab') {
      ...
      navigateCell(td, e.key === 'Tab' && e.shiftKey ? -1 : 1, e.key === 'Enter' ? 'down' : 'right');
    }
    if (e.key === 'Escape') { hideAutocomplete(); cancelCell(td); }
    if (e.key === 'ArrowDown') { e.preventDefault(); commitCell(td, id, key); navigateCell(td, 1, 'down'); }
    if (e.key === 'ArrowUp') { e.preventDefault(); commitCell(td, id, key); navigateCell(td, -1, 'down'); }
  });
}
...
function navigateCell(currentTd, step, direction) {
  ...
  if (direction === 'right') {
    ...
    if (targetTd && targetTd.classList.contains('editable-cell')) activateCell(targetTd);
  }
}
...
// SEARCH / FILTER
// ─────────────────────────────────────────────
['searchBox', 'filterStatus', 'filterFrom', 'filterTo'].forEach(id => {
  document.getElementById(id).addEventListener('input', () => { commitActive(); renderTable(); });
});
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') { cancelCell(activeCell); closeDelete(); }
});
